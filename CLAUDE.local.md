...
**核心问题：** 用户要求的是设计层面的工作，我却跑去逐个读取实体类的每个字段；用户只需要框架层面的方案，我却深入到具体资源的实现细节。这种"过度深入"浪费时间、干扰用户。

**判断规则：** 收到任务后，先判断任务的**抽象层次**，然后严格匹配对应的调查深度：

| 任务抽象层次 | 典型指令 | 允许的调查深度 | 禁止的行为 |
|-------------|---------|--------------|-----------|
| **架构/设计层** | "写个设计方案"、"设计一下数据结构" | 读框架接口、类型枚举、现有设计文档 | ❌ 逐个读取具体实体类的字段定义 |
| **模块/流程层** | "导出流程怎么走的"、"看一下这个模块" | 读 Exporter/Importer 的调用链 | ❌ 读取每个子资源实体的每个字段 |
| **实现层** | "写这个 Exporter 的代码"、"实现导入逻辑" | 读具体实体类、Mapper、SQL | ✅ 这时候才需要深入细节 |

**识别信号——以下情况说明我正在过度深入：**

| # | 过度深入的信号 | 应该做的 |
|---|--------------|---------|
| 1 | 任务是"写设计方案"，我在读 Entity 的每个字段 | 停下来，只需要知道有哪些资源类型和嵌套关系 |
| 2 | 任务是"改设计文档"，我在启动 Agent 读具体实体类 | 停下来，设计文档不需要字段级别的信息 |
| 3 | 用户说了"不需要看具体实现"，我还在读源码 | 立刻停止，用已有信息完成任务 |
| 4 | 我读了 3 个以上不直接相关的实体类 | 反思：这些信息对当前任务真的必要吗？ |

**执行规则：**
```
收到任务 → 判断抽象层次 → 只调查对应深度的信息 → 完成任务
                ↓
          如果不确定需要多深 → 先用最浅层次的信息尝试完成
                ↓
          信息确实不够 → 再按需深入，不要一开始就全部展开
```
...
参照别人的代码之前，必须先回答：**当前要解决的问题属于哪一层？**

```
收到"参照 XX 实现 YY"的指令
│
├── 第一步：判断 YY 问题属于哪一层（查询层？打印层？Controller 层？Service 层？）
│
├── 第二步：只看参照对象中该层对应的代码
│
└── 禁止：把参照对象其他层的实现方案也搬过来
```
...
| 指令 | ✅ 正确 | ❌ 错误 |
|------|--------|--------|
| "参照 `printDetailLedgerNew` 实现打印分页" | 判断"这是打印层问题" → 只看 `GlBookPrinter.java` 的打印逻辑 | 直接照搬参照对象的查询层方案（AllLevel 多次查询） |
| "参照 `queryXxx` 实现 `queryYyy`" | 判断"这是查询层问题" → 只看查询相关代码 | 顺带把打印、导出逻辑也一起加上 |
